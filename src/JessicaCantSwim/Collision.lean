...
private def detect(rect1: Shape.Rectangle) (rect2: Shape.Rectangle): Bool :=
  rect1.x < rect2.x + rect2.width &&
  rect1.x + rect1.width > rect2.x &&
  rect1.y < rect2.y + rect2.height &&
  rect1.y + rect1.height > rect2.y

def detects {ModelID: Type} (entities: List (ModelID × List Shape.Rectangle)): Id (List (ModelID × ModelID)) := do
  let mut collisions := #[]
